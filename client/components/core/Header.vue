...
export default {
  computed: {
    ...
    avatar() {
      return this.$store.getters['admin/avatar']
    },
    auth() {
      return this.$store.getters['admin/auth']
    }
  },
  methods: {
    toggleSideBar() {
      this.$store.dispatch('admin/app/toggleSideBar')
    },
    async logout() {
      await this.$store.dispatch('admin/auth/logout')
